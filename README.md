...
- **자동 알림** — 설정한 주기마다 무료 게임을 수집하고, 서버별로 지정한 시각에 매일 1회 공지합니다.
- **서버별 알림 시각** — `/notifytime` 으로 서버마다 알림 받을 시각(시, KST)을 다르게 설정할 수 있습니다. (기본 12시)
...
# 무료 게임 수집(스크랩) 주기 (시간 단위, 기본값: 6)
# 실제 채널 공지 시각은 서버별로 /notifytime 으로 설정합니다 (기본 매일 12시 KST)
...
| `/notifytime [시각]` | 이 서버의 알림 시각(0-23시, KST) 설정. 인자 없이 실행 시 현재 설정 표시 (기본 12시) |
...
3. (선택) `/notifytime hour:9` 처럼 알림 받을 시각을 설정합니다. 미설정 시 매일 12시(KST)에 공지됩니다.
4. 이후 매일 설정한 시각에 새 무료 게임이 자동으로 공지됩니다.
